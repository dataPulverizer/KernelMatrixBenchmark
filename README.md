...
In terms of raw performance on this task Julia was the winner for the larger data objects with D a very close second but D was the best for smaller data - but both are  *very* close, my feeling is that adding SIMD support and an alternative implementation of my D Matrix object using a different memory model could easily put D ahead. This exercise reveals that Julia's label as a high performance language is more than just hype it has held it's own against highly competitve languages. It was quite hard to get competitve performance from Chapel which I did not expect - it took a lot of investigation from the Chapel team to come up with the current solution, however it is early in the development of this language and we should look forward to further improvements.
