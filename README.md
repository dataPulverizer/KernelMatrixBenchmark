...
It seems that each time you turn around there is a new programming language that is aimed at solving some specific problem set. The increasing proliferation of programming languages and of data are deeply connected in a fundamental way and increasing demand for “data science” computing is a related phenomenon. In the field of scientific computing the Chapel, D, and Julia  are highly relevant programming languages. They arise from different needs and aimed at different problem sets. Chapel focuses on data parallelism on single multicore machines and large clusters, D initially was developed as a more productive safer alternative for C++ and Julia was developed for technical and scientific computing and aimed at getting the best of both worlds from static programming languages which have high performance and the flexibility of dynamic programming languages. However all of them emphasize performance as part of their features. In this article, we look at how their performance varies over kernel matrix calculations, investigate different approaches to code optimization and other usability features of the languages.

Kernel matrix calculations form the basis of many kernel methods in machine learning applications. They scale rather poorly `O(m n^2)` where `n` is the number of items and `m` is the number of elements in each item, in this case `m` will be constant and we will be looking at what happens to execution times as `n` increases. Here `m = 784`, and `n = 1k, 5k, 10k, 20k, 30k`. Each calculation is run 3 times and an average is taken, this approach will suffice for our purpose. We disallow any use of BLAS and only allow use of packages from the language standard library. A reference for kernel functions is located [here](http://crsouza.com/2010/03/17/kernel-functions-for-machine-learning-applications/).

I’d like to say a big thank you to the Chapel, D, and Julia communities for their help in preparing the optimized code all were very patient and helpful in my enquiries.
